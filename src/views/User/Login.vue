    ...
    <div class="bg">
        <div class="login-wrap animated flipInY">
            ...
            <a-form :form="form" @submit="login">
                <a-form-item>
                    <a-input v-decorator="decorator.login" placeholder="用户名">
...
<script>
    export default {
        ...
        data() {
            return {
                decorator: {
                    login: ['login', {rules: [{required: true, message: '请输入用户名!'}]}],
                        ...
                        if (!err) {
                            this.$axios({
                                ...
                                if (res.data.msg == "success") {
                                    $cookies.set("token",res.data.data.id,0);
                                    $cookies.set("username",res.data.data.username,0);
                                    this.$router.push("/")
                                } else {
                                    alert("用户名或密码错误！")
                                    return false;
